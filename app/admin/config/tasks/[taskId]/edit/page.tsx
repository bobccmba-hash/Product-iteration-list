...
export default function TaskEditPage() {
  ...
  const base = useMemo(() => mockTasks.find((t) => t.id === taskId) ?? null, [taskId])
  const { toasts, push, remove } = useToasts()

  const [form, setForm] = useState<FormState>(() => ({
    name: base?.name ?? '',
    desc: '',
    cycle: (base?.cycle as Cycle) ?? 'daily',
    metric: '互动次数',
    targetValue: 3,
    rewardType: 'card',
    rewardValue: 1,
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <div className="mt-2 flex flex-wrap items-center gap-2">
            <span className="font-mono text-xs text-slate-500">{taskId}</span>
          ...
          <div className="space-y-3">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <Field label="统计指标">
                ...
                {selectedMetric && <p className="mt-1 text-xs text-slate-500">{selectedMetric.hint}</p>}
              </Field>
              <Field label="目标数值">
                <input
                  type="number"
